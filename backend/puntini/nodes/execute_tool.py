"""Execute tool node implementation (merged route_tool + call_tool).

This module implements the execute_tool node that validates and executes tools
in one atomic operation, eliminating the unnecessary indirection of the 
separate route_tool and call_tool nodes as specified in Phase 4 of the
...
def execute_tool(
    ...
    This node combines the functionality of route_tool and call_tool nodes,
    eliminating unnecessary indirection as specified in Phase 4 of the 
    progressive refactoring plan. It performs both tool validation and
    execution in one atomic operation.
    
    Args:
        state: Current agent state with tool signature information.
    ...
    Notes:
        ...
        - Atomic validation + execution flow
        - Graph-aware validation when available
    ...
    try:
        ...
        # Perform graph-aware validation if graph store is available
        # This addresses the Claude Graph Critics concerns about entity resolution
        if graph_store is not None:
            # Check if the tool involves entity operations and validate against graph
            if tool_name in ["add_node", "update_props", "delete_node"]:
                try:
                    # Validate entity references against the graph if needed
                    if "match_spec" in tool_args:
                        match_spec = tool_args["match_spec"]
                        # Check if referenced entities exist in the graph
                        # This helps prevent invalid operations
                        existing_entities = _validate_entity_refs(graph_store, match_spec)
                        if not existing_entities and tool_name == "update_props":
                            logger.warning(f"Entity referenced in match_spec may not exist: {match_spec}")
                except Exception as e:
                    logger.warning(f"Could not perform graph-aware validation: {str(e)}")
        
        # Validate args (fast)
        try:
            import inspect
            # Check if tool has a validation method
            if hasattr(tool, 'validate_args'):
                validation_result = tool.validate_args(tool_args)
                if not validation_result.valid:
                    error_msg = f"Tool validation failed: {validation_result.errors}"
                    logger.error(error_msg)
                    return ExecuteToolResponse(
                        current_step="diagnose",
                        result=ExecuteToolResult(
            ...
            logger.warning(f"Could not validate arguments for tool {tool_name}: {str(e)}")
        ...
        # Create routing decision for backward compatibility
        routing_decision = {
        ...
        if normalized_result.status != "success":
            return ExecuteToolResponse(
                ...
                artifacts=[Artifact(type="tool_execution", data={
                    "tool_name": tool_name, 
                    "execution_time": execution_time, 
                    "status": "error", 
        ...
        logger.error(error_msg)
        execution_time = time.time() - start_time
        # Determine if this error is retryable based on the error type
        is_retryable = _is_error_retryable(str(e), tool_name)
        error_type = "tool_error_retryable" if is_retryable else "tool_error"
        return ExecuteToolResponse(
            current_step="diagnose",
            result=ExecuteToolResult(
                status="error",
            ...
            error_context=ErrorContext(
                type="tool_error",
                message=error_msg,
                details={"tool_name": tool_name, "original_error": str(e), "retryable": is_retryable}
            )
        )
        ...
        # Determine if this error is retryable based on the error type
        is_retryable = _is_error_retryable(str(e), tool_name)
        error_type = "system_error_retryable" if is_retryable else "system_error"
        return ExecuteToolResponse(
            current_step="diagnose",
            result=ExecuteToolResult(
                status="error",
...
def _validate_entity_refs(graph_store, match_spec: Dict[str, Any]) -> bool:
    """Validate entity references against the graph store.
    
    Args:
        graph_store: Graph store to validate against
        match_spec: Specification for matching entities
        
    Returns:
        True if entities exist, False otherwise
    """
    # For now, just basic validation - in a real implementation we would query
    # the actual graph to check if referenced entities exist
    try:
        # Check if label and key exist as a basic validation
        if "label" in match_spec and "key" in match_spec:
            # Try to find the node in the graph
            result = graph_store.get_node(match_spec["label"], match_spec["key"])
...
def _is_error_retryable(error_msg: str, tool_name: str) -> bool:
    """Determine if an error is retryable based on error message and tool type.
    ...
    retryable_patterns = [
        ...
        "congestion", "throttled", "rate limit"
    ]
    ...
    # Some tools are inherently more likely to be retryable
    retryable_tools = ["query", "cypher", "search"]
...
def _normalize_tool_result(raw_result: Any, tool_name: str, execution_time: float) -> ExecuteToolResult:
    ...
    Returns:
        Normalized ExecuteToolResult object.
    """
...
def _summarize_result(result: ExecuteToolResult) -> str:
...
def _create_detailed_progress_message(tool_name: str, tool_args: Dict[str, Any], result: ExecuteToolResult) -> str:
