...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    const newDisplayMessages = sessionMessages.map((msg: SessionMessage): DisplayMessage => {
      let content = '';
      let source = 'User';
      ...
      if (msg.message_type) {
        ...
        type = msg.message_type === 'user' ? 'UserMessage' : 'AssistantMessage';
      }
  ...
  const handleSend = async () => {
    if (input.trim() !== '' && currentSession && !messageLoading) {
      try {
        ...
        setError(err instanceof SessionAPIError ? err.message : 'Failed to send message');
      }
    }
  };
  ...
  if (sessionError || error) {
    return (
      <div className="flex flex-col h-full p-4 items-center justify-center">
        <div className="text-lg text-red-600">Error: {sessionError?.message || error}</div>
        <Button 
  ...
  return (
    <div className="flex flex-col h-full p-4">
      ...
      <div className="flex items-center gap-4">
        ...
        />
        <Button onClick={handleSend} disabled={messageLoading || !currentSession}>Send</Button>
      </div>
    </div>
  );
};
