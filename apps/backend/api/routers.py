...
session_router = APIRouter(
    prefix="/sessions", tags=["sessions"], redirect_slashes=False
)
...
async def create_session(
    ...
    if user_key in _session_creation_times:
        ...
        if time_since_last < 1.0:  # 1 second cooldown
            logger.warning(
                f"Rate limiting session creation for user {request.user_id} - too frequent"
            )
            raise HTTPException(
                status_code=429,
    ...
    try:
        logger.info(
            f"Creating session for user: {request.user_id}, project: {request.project_id}"
        )
...
async def get_session_stats(
    ...
    try:
        ...
        completed_sessions = [
            s for s in sessions if s.status.value in ["expired", "cleaning_up"]
        ]
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get session stats: {e!s}"
        ) from e
...
async def destroy_session(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to destroy session: {e!s}"
        ) from e
...
async def receive_messages(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to receive message: {e!s}"
        ) from e
...
async def get_project_context(
    ...
    try:
        ...
        return {"project_context": context, "tasks": tasks, "task_count": len(tasks)}
    except SessionNotFoundError as e:
        raise HTTPException(status_code=404, detail="Session not found") from e
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get context: {e!s}"
        ) from e
...
async def update_project_context(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update context: {e!s}"
        ) from e
...
async def get_tasks(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get tasks: {e!s}"
        ) from e
